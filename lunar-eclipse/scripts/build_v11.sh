...
for i in 1 2 3 4 5 6 7 8; do
  ffmpeg -y -v error -i "/home/user/v10/voices/raw$i" -ar 48000 -ac 2 "w6/$L/voices/voice$i.wav"
done
declare -A REVEAL=( [4]="6.0,11.5" [5]="12.0" [7]="3.0" )
for i in 1 2 3 4 5 6 7 8; do
  python3 tighten.py "w6/$L/voices/voice$i.wav" "w6/$L/voices/tg$i.wav" 0.30 0.14 "${REVEAL[$i]:-}"
  mv "w6/$L/voices/tg$i.wav" "w6/$L/voices/voice$i.wav"
done
echo "TIGHTEN_OK"
...
TRIM="silenceremove=start_periods=1:start_threshold=-45dB:start_silence=0.10,areverse,silenceremove=start_periods=1:start_threshold=-45dB:start_silence=0.10,areverse"
CUTS=""
for i in 1 2 3 4 5 6 7 8; do
  ...
  SRC="w6/clips/clip$i.mp4"
  LEN=$(python3 -c "print(f'{$D+0.06:.3f}')")
  # A dense line on a short clip would need extreme slow motion, which kills the pace.
  # Ping-pong the shot instead (forward + reverse) so motion stays near natural speed.
  NEED=$(python3 -c "print(f'{$LEN/$C:.4f}')")
  if python3 -c "import sys; sys.exit(0 if $NEED>1.45 else 1)"; then
    ffmpeg -y -v error -i "w6/clips/clip$i.mp4" -filter_complex \
      "[0:v]split[a][b];[b]reverse[r];[a][r]concat=n=2:v=1[v]" -map "[v]" -an \
      -c:v libx264 -preset medium -crf 18 "w6/clips/pp$i.mp4"
    SRC="w6/clips/pp$i.mp4"
    C=$(ffprobe -v error -show_entries format=duration -of csv=p=0 "$SRC")
    echo "  pingpong clip$i -> ${C}s (needed ${NEED}x)"
  fi
  R=$(python3 -c "print(f'{max($LEN/$C,0.70):.5f}')")
  ffmpeg -y -v error -i "w6/$L/voices/trim$i.wav" -af "adelay=40|40,apad" -t "$LEN" -ar 48000 -ac 2 "w6/$L/seg/a$i.wav"
...
python3 seams.py "w6/$L/out/vo.wav" "$(echo $CUTS | tr ' ' ',')" 1.0
python3 mksfx_bed.py "w6/$L/out/vo.wav" "$DUR" "$CUTCSV2" "w6/$L/out/sfxbed.wav" "w6/$L/out/reveals.txt"
MUSVOL=$(python3 -c "
rv=[float(x) for x in open('w6/$L/out/reveals.txt').read().split(',') if x.strip()]
d=''.join(f'-0.13*exp(-pow(t-{r-1.1:.2f}\\,2)/0.45)' for r in rv)
u=''.join(f'+0.16*exp(-pow(t-{r+0.7:.2f}\\,2)/1.10)' for r in rv)
print(f'0.30{d}{u}')")
echo "MUSVOL=$MUSVOL"
ffmpeg -y -v error -stream_loop -1 -i w6/music.m4a -ar 48000 -ac 2 -t "$DUR" \
  -af "afade=t=in:st=0:d=1.0,afade=t=out:st=$FO:d=2.2,volume=eval=frame:volume='$MUSVOL'" "w6/$L/out/bed.wav"
ffmpeg -y -v error -i "w6/$L/out/bed.wav" -i "w6/$L/out/vo.wav" -i "w6/$L/out/sfxbed.wav" -filter_complex \
  "[0:a][1:a]sidechaincompress=threshold=0.028:ratio=7:attack=8:release=420[d];[2:a][1:a]sidechaincompress=threshold=0.05:ratio=4:attack=5:release=260[sx];[1:a][d][sx]amix=inputs=3:duration=first:normalize=0,loudnorm=I=-14:TP=-1.5:LRA=11[m]" \
