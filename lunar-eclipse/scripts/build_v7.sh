...
hf_20260826_144237_48fdaed4-fbcc-4c87-b2a2-f1632dc625a2.wav \
hf_20260825_180815_32afcd98-1d36-4bf0-abc1-266cd7dc3835.wav \
hf_20260825_180814_3b828b45-8f9e-4a65-beab-7945edc61673.wav \
hf_20260826_144236_c2b860ec-29d9-4098-8ea6-24da2b9e8945.wav \
...
if [ "$L" = "en" ]; then EXT=wav; else EXT=mp3; fi
for i in 1 2 3 4 5 6 7 8; do
  [ -s "w6/clips/clip$i.mp4" ] || curl -sSf -o "w6/clips/clip$i.mp4" "$B/hf_20260825_173807_${CL[$((i-1))]}.mp4" &
  if [ "$L" = "en" ]; then
    curl -sSf -o "w6/$L/voices/voice$i.$EXT" "$B/${EN[$((i-1))]}" &
  else
    curl -sSf -o "w6/$L/voices/voice$i.$EXT" "$B/hf_20260825_184603_${TA[$((i-1))]}.mp3" &
  fi
done; wait
...
echo "DL_OK"
# Block 2 comes from text2speech_v2/minimax, not seed_audio: seed_audio kept
# auto-completing the line back into the cliche "the last full moon of summer".
if [ "$L" = "en" ]; then
  curl -sSf -o mm2.mp3 "https://d8j0ntlcm91z4.cloudfront.net/user_3FXuOlg1HqS8yvURHbzZgj2jNLj/hf_20260826_145051_3e3d4693-f793-4f60-b342-e57c475b69c8.mp3"
  ffmpeg -y -v error -i mm2.mp3 -ar 48000 -ac 2 w6/en/voices/voice2.wav
  echo "BLOCK2_MINIMAX_OK"
fi
