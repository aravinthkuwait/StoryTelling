...
# v8 narration: 06 Hard-Cut Thriller (blocks 1-6), 08 Direct Address (blocks 7-8)
for i in 1 2 3 4 5 6 7 8; do
  ffmpeg -y -v error -i "/home/user/v8/voices/raw$i" -ar 48000 -ac 2 "w6/$L/voices/voice$i.wav"
done
# trailer staccato the TTS will not hold: hard gaps on the punch beats
python3 staccato.py w6/en/voices/voice1.wav w6/en/voices/sc1.wav moon:1:0.24 turns:1:0.24
mv w6/en/voices/sc1.wav w6/en/voices/voice1.wav
python3 staccato.py w6/en/voices/voice4.wav w6/en/voices/sc4.wav surface:1:0.26 you:1:0.30 telescope:1:0.24
mv w6/en/voices/sc4.wav w6/en/voices/voice4.wav
python3 staccato.py w6/en/voices/voice6.wav w6/en/voices/sc6.wav rise:1:0.26 africa:1:0.34
mv w6/en/voices/sc6.wav w6/en/voices/voice6.wav
echo "STACCATO_OK"
...
if [ ! -s w6/s0.mp4 ]; then
ffmpeg -y -v error -ss 0 -t 1.4 -i w6/card.mp4 -i title_text.png -f lavfi -t 1.4 -i anullsrc=r=48000:cl=stereo \
  -filter_complex "[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,fps=30[c];[c][1:v]overlay=0:0,format=yuv420p,fade=t=in:st=0:d=0.15,fade=t=out:st=1.2:d=0.2[v]" \
  -map "[v]" -map 2:a -c:v libx264 -preset medium -crf 18 -c:a aac -b:a 192k -shortest w6/s0.mp4
ffmpeg -y -v error -ss 2.6 -t 1.8 -i w6/card.mp4 -i title_text.png -f lavfi -t 1.8 -i anullsrc=r=48000:cl=stereo \
...
for i in 1 2 3 4 5 6 7 8; do
  ...
  R=$(python3 -c "print(f'{max($LEN/$C,1.0):.5f}')")
...
python3 mkbeats.py w6/music.m4a "$SDUR" 1.4 "$CUTCSV" "w6/$L/out/fx2.txt"
